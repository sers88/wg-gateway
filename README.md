...
wg-easy v15 supports two setup modes:

### Unattended setup (recommended)

If `WG_HOST` and `WG_EASY_INIT_PASSWORD` are set in `.env`, the container auto-configures on first start:

1. Start the container.
2. Open the **wg-easy UI** at `http://YOUR_SERVER_IP:51821` and log in with the credentials from `.env`.
...
If `WG_HOST` is not set or `WG_EASY_INIT_PASSWORD` is empty, the Web UI setup wizard runs on first start. Open `http://YOUR_SERVER_IP:51821` and follow the on-screen steps.

> **Note:** By default, **all traffic goes DIRECT** (no proxy). To route traffic through a proxy, edit the Mihomo config (see below).
...
|---|---|---|
| `WG_HOST` | _(none — required)_ | Public IP or hostname for client configs. Also used as `INIT_HOST` for unattended wg-easy setup. |
| `WG_EASY_INIT_PASSWORD` | _(none — required for unattended setup)_ | Plaintext password for the wg-easy admin account (created once on first start). Not a bcrypt hash. |

### Optional

| Variable | Default | Description |
|---|---|---|
| `WG_PORT` | `51820` | WireGuard server UDP port (used for `INIT_PORT`) |
| `WG_DEFAULT_DNS` | `1.1.1.1` | DNS advertised to WireGuard clients (used for `INIT_DNS`) |
| `WG_ALLOWED_IPS` | `0.0.0.0/0,::/0` | Routes advertised to clients (used for `INIT_ALLOWED_IPS`) |
| `WG_EASY_PORT` | `51821` | wg-easy Web UI port |
| `WG_EASY_INIT_USERNAME` | `admin` | Admin username for wg-easy (used once on first start) |
| `INSECURE` | `true` | Allow HTTP access to wg-easy UI (set to `false` only if behind an HTTPS reverse proxy) |
...
6. Set `WG_HOST` to your public IP and `WG_EASY_INIT_PASSWORD` to a secure password.
